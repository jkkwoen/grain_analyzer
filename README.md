...
A Python package for grain (particle) detection and analysis from AFM/XQD images (Version 0.2.0)

## Features

- **Multiple Segmentation Methods**
  - Classical: Otsu thresholding + Distance Transform + Voronoi
...
- **Statistics Calculation**: Detailed statistics for each grain (area, diameter, centroid, peak position, etc.)
...
```bash
# For StarDist
pip install "grain-analyzer[stardist]"

# For CellPose
pip install "grain-analyzer[cellpose]"

# Install all optional dependencies
pip install "grain-analyzer[all]"
```
...
from grain_analyzer.afm_data_wrapper import AFMData
...
labels = segment_classical(height, meta)  # Default method
...
```python
from grain_analyzer import print_gpu_info, setup_gpu_environment
...
```python
from grain_analyzer import CellulusTrainer, TrainingConfig, setup_environment
...
- `segment_classical(height, meta, **kwargs)`: Classical segmentation
