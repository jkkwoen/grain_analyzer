#!/usr/bin/env python3
"""
Grain Analyzer 모델 학습 파이프라인

AFM grain 데이터로 세그멘테이션 모델을 학습합니다.
...
class TrainingConfig:
    ...
    def __post_init__(self):
        if self.data_dir is None:
            self.data_dir = Path(__file__).parent.parent / "tests" / "input_data" / "qd"
        if self.output_dir is None:
            self.output_dir = Path(__file__).parent.parent / "tests" / "model_data" / self.model_type
...
def load_afm_data(data_dir: Path) -> List[np.ndarray]:
    """AFM XQD 파일들 로드"""
