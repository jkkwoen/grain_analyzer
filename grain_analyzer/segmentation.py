...
사용 예시:
    >>> from grain_analyzer.segmentation import segment_classical, segment_stardist, segment_cellpose
...
def segment_classical(
    ...
    *,
    ...
    """
    Classical segmentation: Otsu + Distance Transform + Voronoi
    ...
    Examples
    --------
    >>> labels = segment_classical(height, meta, gaussian_sigma=1.5)
...
def segment_stardist(
    ...
    *,
    ...
    # Load model
    if model_path:
        model = StarDist2D(None, name='custom', basedir=model_path)
    else:
...
def segment_cellulus(
    ...
    *,
    ...
    1. 공식 Cellulus 모델 (cellulus 패키지 필요)
    2. 간소화된 모델 (train_model.py로 학습, PyTorch만 필요)
    ...
    Notes
    -----
    - Train your model with: python train_model.py
    ...
    try:
        ...
        raise RuntimeError(f"Cellulus 추론 실패: {e}")
